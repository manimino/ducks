...
def safety(session: Session) -> None:
    ...
    # safety is erroring on some numpy vulnerabilities. Fixing them requires going python >=3.8
    # once numpy is upgrades to at least 1.2.2, remove --continue-on-error
    session.run(
        "safety",
        "check",
        "--full-report",
...
def bandit(session: Session) -> None:
    ...
    args = session.posargs or ["-r", "./ducks"]
