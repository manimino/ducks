...
@pytest.mark.parametrize('end_full, expected_len', [
    (True, 10),
    (False, 0),
])
def test_add_remove(priority, end_full, expected_len):
    ...
    while time.time() - t0 < duration:
        t1 = threading.Thread(target=worker_add_remove, args=[objs, box], kwargs={'end_full': end_full})
        t2 = threading.Thread(target=worker_add_remove, args=[objs, box], kwargs={'end_full': end_full})
        t1.start()
        t2.start()
        t1.join()
        t2.join()
